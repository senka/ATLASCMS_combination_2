...
l2nu2_bkgDD=[0,0,0,0,17.48, 7.58, 0.78]
...
syst_signal_reco_err=[0.79, 0.55, 0.37, 0.07, 0.27, 0.48, 0.4]
...
def isItCorrelated(name):
    ...
    if ('_eff_b' in name or '_les' in name or '_pu' in name or '_umet' in name or '_res_j' in name or '_scale_j' in name ):
        print '-> true'
        return True
    else:
        print '-> false'
...
for section in fit_sections:
    ...
    lType = codename
    print '\n\tlType=',lType
    f = TFile('%s/%s_boosted_withSignalSyst.root'%(basepath,codename))
    ...
    for j in range(0,Nbkg_int):
        ...
        for i in range(0,len(background_shapeSyst[j])):
            ...
            print ' yield: ',(f.Get('%sUp'%background_shapeSyst[j][i])).Integral()
...
#                              '%s/signal_WV_%s.root'%(basepath,codename))
    ...
    aTGCPdf = RooATGCSemiAnalyticPdf_wz('ATGCPdf_WWgammaZ_WV_atgc_semileptonic_%s'%codename,
                                        ...
                                        dg1,
                                        dibosonPdf,
                                        '%s/signal_%s_f5.root'%(basepath,codename),
                                        limtype
                                        )
...
lumi_8TeV       \t    lnN \t    1.039             """
    for i in range(0,Nbkg_int):
        if (i==0 and (codename=="ch5" or codename=="ch6" or codename=="ch7")): card += """\t\t\t\t1.039""".format(codename=codename,norm_sig_sm=norm_sig_sm,norm_bkg=norm_bkg,norm_obs=norm_obs)
        else: card += """\t\t\t\t-""".format(codename=codename,norm_sig_sm=norm_sig_sm,norm_bkg=norm_bkg,norm_obs=norm_obs)

    card += """           
sig_stat_{codename}       \t    lnN \t    {down}/{up}             """.format(codename=codename,up=(stat_signal_err[ch]+norm_sig_sm)/norm_sig_sm,down=(norm_sig_sm-stat_signal_err[ch])/norm_sig_sm)
    for i in range(0,Nbkg_int):
        card += """\t\t\t\t-""".format(codename=codename,norm_sig_sm=norm_sig_sm,norm_bkg=norm_bkg,norm_obs=norm_obs)

    card += """           
sig_reco       \t    lnN \t    {down}/{up}             """.format(codename=codename,up=(syst_signal_sum_err[ch]+norm_sig_sm)/norm_sig_sm,down=(norm_sig_sm-syst_signal_sum_err[ch])/norm_sig_sm)
    for i in range(0,Nbkg_int):
        card += """\t\t\t\t-""".format(codename=codename,norm_sig_sm=norm_sig_sm,norm_bkg=norm_bkg,norm_obs=norm_obs)

# th error but use the same name since they are correlated!
#    card += """           
#sig_reco       \t    lnN \t    {down}/{up}             """.format(codename=codename,up=(syst_signal_th_err[ch]+norm_sig_sm)/norm_sig_sm,down=(norm_sig_sm-syst_signal_th_err[ch])/norm_sig_sm)
#    for i in range(0,Nbkg_int):
#        card += """\t\t\t\t-""".format(codename=codename,norm_sig_sm=norm_sig_sm,norm_bkg=norm_bkg,norm_obs=norm_obs)

    card += """           
bkgDD_stat_{codename}       \t    lnN \t    -             """.format(codename=codename)
    for i in range(0,Nbkg_int):
        if (i==0 and (codename=="ch5" or codename=="ch6" or codename=="ch7")): 
            card += """\t\t\t\t-""".format(codename=codename,norm_sig_sm=norm_sig_sm,norm_bkg=norm_bkg,norm_obs=norm_obs,up=(syst_bkgDD_err[ch]+norm_bkg[i])/norm_bkg[i],down=(norm_bkg[i]-syst_bkgDD_err[ch])/norm_bkg[i])
        elif ((norm_bkg[i]-stat_bkgDD_err[ch])/norm_bkg[i]>0):
            card += """\t\t\t\t{down}/{up}""".format(codename=codename,norm_sig_sm=norm_sig_sm,norm_bkg=norm_bkg,norm_obs=norm_obs,up=(stat_bkgDD_err[ch]+norm_bkg[i])/norm_bkg[i],down=(norm_bkg[i]-stat_bkgDD_err[ch])/norm_bkg[i])
        else:
            card += """\t\t\t\t{down}/{up}""".format(codename=codename,norm_sig_sm=norm_sig_sm,norm_bkg=norm_bkg,norm_obs=norm_obs,up=(stat_bkgDD_err[ch]+norm_bkg[i])/norm_bkg[i],down=0.001)

    card += """           
bkgMC_stat_{codename}       \t    lnN \t    -             """.format(codename=codename)
    for i in range(0,Nbkg_int):
        if (i==0 and (codename=="ch5" or codename=="ch6" or codename=="ch7")):
            card += """\t\t\t\t{down}/{up}""".format(codename=codename,norm_sig_sm=norm_sig_sm,norm_bkg=norm_bkg,norm_obs=norm_obs,up=(stat_bkgMC_err[ch]+norm_bkg[i])/norm_bkg[i],down=(norm_bkg[i]-stat_bkgMC_err[ch])/norm_bkg[i])
        else:
            card += """\t\t\t\t-"""
            

    card += """           
bkgDD_syst_{codename_forBKGDDsyst}        \t    lnN \t    -             """.format(codename_forBKGDDsyst=codename_forBKGDDsyst)
    for i in range(0,Nbkg_int):
        if (i==0 and (codename=="ch5" or codename=="ch6" or codename=="ch7")): 
            card += """\t\t\t\t-""".format(codename=codename,norm_sig_sm=norm_sig_sm,norm_bkg=norm_bkg,norm_obs=norm_obs,up=(syst_bkgDD_err[ch]+norm_bkg[i])/norm_bkg[i],down=(norm_bkg[i]-syst_bkgDD_err[ch])/norm_bkg[i])
        else: 
            card += """\t\t\t\t{down}/{up}""".format(codename=codename,norm_sig_sm=norm_sig_sm,norm_bkg=norm_bkg,norm_obs=norm_obs,up=(syst_bkgDD_err[ch]+norm_bkg[i])/norm_bkg[i],down=(norm_bkg[i]-syst_bkgDD_err[ch])/norm_bkg[i])
        

    card += """           
bkgMC_syst       \t    lnN \t    -             """.format(codename=codename)
    for i in range(0,Nbkg_int):
        if (i==0 and (codename=="ch5" or codename=="ch6" or codename=="ch7")):
            card += """\t\t\t\t{down}/{up}""".format(codename=codename,norm_sig_sm=norm_sig_sm,norm_bkg=norm_bkg,norm_obs=norm_obs,up=(syst_bkgMC_err[ch]+norm_bkg[i])/norm_bkg[i],down=(norm_bkg[i]-syst_bkgMC_err[ch])/norm_bkg[i])
        else:
             card += """\t\t\t\t-"""
