...
lumi_8TeV       \t    lnN \t    1.039             """
    for i in range(0,Nbkg_int):
        if (i==2): card += """\t\t\t\t1.039""".format(codename=codename,norm_sig_sm=norm_sig_sm,norm_bkg=norm_bkg,norm_obs=norm_obs)
...
#{background_shapeSyst} shape  """.format(codename=codename,norm_sig_sm=norm_sig_sm,norm_bkg=norm_bkg,norm_obs=norm_obs,i=i,background_shapeSyst=background_shapeSyst[j][i])
                ...
                if (background_backshapeUp_yield[j][i]>0.001 and background_backshapeDown_yield[j][i]>0.001):
                    card += """{down}/{up} """.format(up=background_backshapeUp_yield[j][i],down=background_backshapeDown_yield[j][i])
                elif (background_backshapeUp_yield[j][i]<0.001):
                    card += """{down}/{up} """.format(up=0.001,down=background_backshapeDown_yield[j][i])
                elif (background_backshapeDown_yield[j][i]<0.001):
                    card += """{down}/{up} """.format(up=background_backshapeUp_yield[j][i],down=0.001)
                elif (background_backshapeDown_yield[j][i]<0.001 and background_backshapeUp_yield[j][i]<0.001):
                    card += """{down}/{up} """.format(up=0.001,down=0.001)
    ...
    if (doSignalShape_unc):
        for i in range(0,len(signal_shapeSyst)):
            ...
            if (isItCorrelated(str(signal_shapeSyst[i]))):
                ...
                print '@@@@@@@@@@@@@@ testing ',norm_sig_sm_up[i]/norm_sig_sm,' ',norm_sig_sm_down[i]/norm_sig_sm
                if ((norm_sig_sm_up[i]/norm_sig_sm)>0.001 and (norm_sig_sm_down[i]/norm_sig_sm)>0.001):
                    print '@@@@@@@@@@@@@@ testing defout'
                    card += """
{signal_shapeSyst}        lnN   {down}/{up}         """.format(signal_shapeSyst=name_forCorr,up=norm_sig_sm_up[i]/norm_sig_sm,down=norm_sig_sm_down[i]/norm_sig_sm)
                elif (norm_sig_sm_up[i]/norm_sig_sm<0.001):
                    print '@@@@@@@@@@@@@@ testing up<0.001'
                    card += """
{signal_shapeSyst}        lnN   {down}/{up}         """.format(signal_shapeSyst=name_forCorr,up=0.001,down=norm_sig_sm_down[i]/norm_sig_sm)
                elif (norm_sig_sm_down[i]/norm_sig_sm<0.001):
                    print '@@@@@@@@@@@@@@ testing down<0.001'
                    card += """
{signal_shapeSyst}        lnN   {down}/{up}         """.format(signal_shapeSyst=name_forCorr,up=norm_sig_sm_up[i]/norm_sig_sm,down=0.001)
                for j in range(0,Nbkg_int):
                    isItCorr=False
                    i_bkg=-1
                    for k in range(0,len(background_shapeSyst[j])):
                        if (name_forCorr in background_shapeSyst[j][k]):
                            ...
                            i_bkg=k
                    if (isItCorr):
                        card += """\t\t\t\t{down}/{up} """.format(codename=codename,norm_sig_sm=norm_sig_sm,norm_bkg=norm_bkg,norm_obs=norm_obs,up=background_backshapeUp_yield[j][i_bkg],down=background_backshapeDown_yield[j][i_bkg])
                    else:
