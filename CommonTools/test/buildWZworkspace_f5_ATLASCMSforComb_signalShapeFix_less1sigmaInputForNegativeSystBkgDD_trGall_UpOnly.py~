...
background_{codename}boosted_backshape          trG      -            """.format(codename=codename)
        for i in range(0,Nbkg_int):
            card += """\t\t\t\t1.3"""
        card += """           
signal_th          trG      1.04            """.format(codename=codename)
        for i in range(0,Nbkg_int):
            card += """\t\t\t\t-"""
...
lumi_8TeV           trG     1.039\t\t\t-"""
        if (leptons=="l2nu2"):
            card += """
{leptons}_{codename}_background_MC_stat        trG     -                           {err_up}                             -""".format(leptons=leptons,codename=codename,err_down=stat_bkgMC_err_rel_Down[ch],err_up=stat_bkgMC_err_rel_Up[ch])
            card += """
background_MC_syst        trG     -                           {err_up}                             -""".format(leptons=leptons,codename=codename,err_down=syst_bkgMC_err_rel_Down[ch],err_up=syst_bkgMC_err_rel_Up[ch])

        if (leptons=="l2nu2"):
            card += """
{leptons}_{codename}_background_DD_factor2p5_stat        trG     -                           -                             {err_up}""".format(leptons=leptons,codename=codename,err_down=stat_bkgDD_err_rel_Down[ch],err_up=stat_bkgDD_err_rel_Up[ch])

            if (codename_forBKGDDsyst=="odd"):
                card += """
odd_background_DD_syst       trG     -                           -                             {err_up}""".format(leptons=leptons,codename=codename,err_down=syst_bkgDD_err_rel_Down[ch],err_up=syst_bkgDD_err_rel_Up[ch])
            else:
                card += """
even_background_DD_syst       trG     -                           -                             {err_up}""".format(leptons=leptons,codename=codename,err_down=syst_bkgDD_err_rel_Down[ch],err_up=syst_bkgDD_err_rel_Up[ch])
            card += """
{leptons}_{codename}_signal_stat       trG     {err_up}                           -                             -""".format(leptons=leptons,codename=codename,err_down=stat_signal_err_rel_Down[ch],err_up=stat_signal_err_rel_Up[ch])
            card += """
signal_reco       trG     {err_up}                           -                             -""".format(leptons=leptons,codename=codename,err_down=syst_signal_reco_err_rel_Down[ch],err_up=syst_signal_reco_err_rel_Up[ch])
            card += """
signal_th       trG     {err_up}                           -                             -""".format(leptons=leptons,codename=codename,err_down=syst_signal_th_err_rel_Down[ch],err_up=syst_signal_th_err_rel_Up[ch])
            ...
            card += """
{leptons}_{codename}_background_DD_factor1p5_stat        trG     -                             {err_up}""".format(leptons=leptons,codename=codename,err_down=stat_bkgDD_err_rel_Down[ch],err_up=stat_bkgDD_err_rel_Up[ch])
