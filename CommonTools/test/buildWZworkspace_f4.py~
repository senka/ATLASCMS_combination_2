...
#        norm_sig_sm_down = diboson_down.Integral()
    ...
    dkg = theWS.factory('dkg[0.,-0.03, 0.03]')
    dg1 = theWS.factory('dg1[0.,-0.03,0.03]')
    ...
    if (doSignalShape_unc):
        ...
        for i in range(0,len(signal_shapeSyst)):
            ...
            aTGCPdf_norm[i] = AsymPow('ATGCPdf_WWgammaZ_WV_atgc_semileptonic_%s_integral%s'%(codename,i+1),
                                      ...
                                      kappaHigh[i],
                                      theta[i])
#            aTGCPdf_norm_sum = aTGCPdf_norm_sum*aTGCPdf_norm[i]
...
#            prodset.add(aTGCPdf_norm[1])
            aTGCPdf_norm_sum = RooProduct("aTGCPdf_norm_sum","aTGCPdf_norm_sum",prodset)
#            aTGCPdf_norm_sum = RooProduct("aTGCPdf_norm_sum","aTGCPdf_norm_sum",RooArgSet(aTGCPdf_norm[0],aTGCPdf_norm[1]))
...
#        aTGCPdf_norm[i] = AsymPow('ATGCPdf_WWgammaZ_WV_atgc_semileptonic_%s_norm'%codename,
#                                  'ATGCPdf_WV_%s_norm'%codename,
#                                  kappaLow_sum,
#                                  kappaHigh_sum,
#                                  theta_sum)
...
------------
lumi_8TeV           lnN     1.039		      -""".format(codename=codename,norm_sig_sm=norm_sig_sm,norm_bkg=norm_bkg,norm_obs=norm_obs)
